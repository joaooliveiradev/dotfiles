...
1. **Verify setup.** Confirm `.craft/project/` exists. If it doesn't, stop and tell the user to run `/craft init` first.
...
5. **Create the folder:**
   - Spec mode → `mkdir -p .craft/features/[slug]`
   - Quick mode → `mkdir -p .craft/quick/[slug]`
...
7. **Write the slug into STATE.md.**
   - If `STATE.md` does not exist, create `.craft/project/STATE.md` using the minimal template below.
